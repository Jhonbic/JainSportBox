...
- **Indicador de navegación:** como consecuencia de lo anterior, entre que se dispara una navegación y que la vista aparece hay una descarga, y el área de contenido queda en blanco. `lib/navegacion.js` expone `cargandoRuta`, que el router prende en `beforeEach` y apaga en `afterEach` **y en `onError`** (sin lo último, un chunk que no baja —deploy que invalidó el hash, red caída— deja el indicador puesto para siempre). `Dashboard.vue` lo usa para mostrar el logo en lugar del `<router-view>`. **El retraso de 150 ms no es un adorno:** una ruta ya descargada resuelve en milisegundos y un indicador que aparece y se va en ese lapso se lee como un parpadeo, peor que no mostrar nada. Vive en `lib/` y no en `router/index.js` porque el router importa `Dashboard.vue` de forma estática y quedaría un ciclo.
- **`await router.push(...)` al terminar el login.** Sin el `await`, el `finally` apagaba el spinner del botón apenas se disparaba la navegación, pero el chunk de la vista de destino todavía estaba bajando: el botón volvía a "Ingresar" y la pantalla de login se quedaba quieta unos instantes antes de saltar — la sensación exacta de que el sistema se colgó. El indicador de arriba **no** cubre este caso, porque `LoginView` está fuera del layout `Dashboard`.
...
**Exportar Excel:** botón en el header → `GET /finanzas/exportar-excel` (solo admin), con **el período y los filtros seleccionados en pantalla**, no el histórico. Filtrar por "Nómina" y que el archivo salga con todo el mes es la clase de sorpresa que hace desconfiar del export. Dos excepciones deliberadas: la hoja **Resumen** siempre es del período **sin filtrar** (son los totales del negocio; recortarlos por un filtro de pantalla los volvería engañosos), y **`tipo` no viaja** porque el archivo ya trae hojas separadas de Ingresos y Egresos y filtrarlo dejaría una vacía sin motivo visible. Genera **tres hojas**: *Resumen* (período, membresías, tienda, ingresos, egresos, balance + desglose de egresos por categoría, que **se imprime siempre** — omitirlo cuando está vacío hace dudar de si no hubo gastos o si el export se rompió), *Ingresos* (pagos de membresía + ventas + ingresos manuales, con columna **Origen**) y *Egresos* (solo registros manuales, **sin** columna Origen porque diría lo mismo en todas las filas). Las dos hojas de movimientos traen **todas** las filas del período (**sin el tope de 200** del listado: un export recortado sería un export equivocado), montos **positivos** (el nombre de la hoja ya da el signo) y una fila **TOTAL** al pie. Las fechas se convierten de UTC a Bogotá para que coincidan con la pantalla.
...
El endpoint y el listado comparten `_recolectar_movimientos()`, que fusiona pagos + ventas + movimientos manuales. Si esa fusión se duplicara, el Excel y la pantalla podrían divergir. **Los filtros viven dentro de ese helper**, no en el llamador, por el mismo motivo: así el Excel exporta exactamente lo que se ve en pantalla.

**Fechas: `_apply_date_filter` usa `inicio_dia_utc`/`fin_dia_utc` de `fechas.py`.** Acá se armaba la ventana con `datetime.combine(desde, min.time())`, que es medianoche **UTC** — las 19:00 de Bogotá del día anterior. Con el box entrenando de 19:00 a 21:00, "Hoy" arrastraba la noche de ayer y perdía la de hoy. Es el mismo bug que ya se había corregido en `asistencia` y `dashboard`: **no redefinas los helpers localmente**. El frontend es la otra punta — `calcularFechas()` usa `aISO` de `lib/fechas.js` y no `toISOString()`, que también da la fecha UTC. Cubierto por `test_ventana_es_el_dia_de_bogota_no_el_de_utc`, que ataca las dos puntas (21:00 de hoy entra, 20:00 de ayer no).

**Filtros y buscador del historial.** `GET /finanzas/movimientos` acepta `tipo`, `categoria`, `plan_id`, `metodo_pago`, `q`, `skip` y `limit`, y devuelve **`{ items, total, total_monto }`** — no una lista pelada. La paginación es **de servidor**: con el tope de 200 en cliente que había antes, buscar algo que sí está en el período y no verlo aparecer sería peor que no tener buscador. `q` se normaliza sin acentos y en minúsculas (`_sin_acentos`) contra concepto y nombre del cliente, así "nomina" encuentra "Nómina"; va en memoria al final del helper porque en pagos y ventas el concepto es texto armado en Python, no una columna. `plan_id` solo aplica a pagos de membresía: al usarlo se excluyen ventas y movimientos manuales, que no pertenecen a ningún plan.

**Ojo con las categorías del filtro vs las del alta.** `CATEGORIAS_POR_TIPO` (el modal) **no** incluye `venta_tienda` a propósito — no se puede crear una venta a mano. Pero para *filtrar* sí hay que ofrecerla, porque existe en los datos: por eso `CATEGORIAS_FILTRO` la agrega.

**`GET /finanzas/estadisticas` — qué se vendió.** Desglose del período por plan y por producto (`cantidad`, `total`, `porcentaje`), agregado en SQL con `group_by` + `outerjoin` (el `outerjoin` es necesario: un pago personalizado tiene `plan_id = NULL` y con un join normal desaparecería del desglose). **Las dos tablas suman exactamente lo mismo que sus tarjetas de balance**, y eso obligó a incluir una fila **"Ingreso manual (sin plan)"**: la tarjeta "Membresías" suma también los movimientos manuales categorizados como `mensualidad`, que no tienen plan detrás. Sin esa fila, la tabla y la tarjeta mostraban números distintos uno al lado del otro — en una pantalla de plata, lo peor que puede pasar.

El historial pagina de a 15 **contra el servidor**. **Ojo con los nombres:** `rangoDesde`/`rangoHasta` son las fechas del período "Rango"; las filas visibles son `filaDesde`/`filaHasta`. `finanzas.py` surfaces income from three sources: rows in `pagos` (membership payments, both plan-based and personalizado), rows in `ventas` (shop sales), and rows in `movimientos_financieros` (manual entries + legacy `pago_directo` records). Pagos and ventas are NOT mirrored into `movimientos_financieros` — the finanzas listing reads from each table directly to avoid double-counting.
...
**Cartel de resultado.** Con vigencia: foto, nombre grande y el número de días restantes en grande + fecha de vencimiento. Sin vigencia (403): solo **"Membresía vencida"** + "Acércate a recepción para renovar tu mensualidad" — **sin nombre ni fechas, a pedido explícito**; en un mostrador con fila detrás no se expone quién está en mora. Documento inexistente (404): "Documento no encontrado". El helper `_falloDesde(e)` traduce el status a ese cartel, así que **la distinción vive en el status code** y no hace falta detail estructurado en el backend. El resultado se autolimpia a los `SEGUNDOS_RESULTADO` (5 s) para que el siguiente de la fila no vea los datos del anterior; ese es el techo, porque un `watch` sobre `documento` lo borra apenas alguien empieza a teclear la cédula siguiente.

**Flujo de teclado continuo — la pantalla se usa sin mouse.** El PC queda abierto acá mientras el coach entrena, y la gente que llega escribe su cédula y da Enter, uno tras otro. Tres piezas lo sostienen y las tres son fáciles de romper sin darse cuenta:

1. **El input va `:readonly="procesando"`, NUNCA `:disabled`.** Un elemento deshabilitado **pierde el foco**, y recuperarlo después no alcanza: Vue quita el atributo un tick más tarde que el `focus()`, así que el `focus()` cae sobre un input todavía deshabilitado y el navegador lo ignora. Ese fue exactamente el bug — el `// Listo para el siguiente cliente` estaba escrito desde el principio y nunca funcionó, y en recepción había que volver al campo con el mouse en cada persona de la fila. El doble envío lo frena el guard de `registrarAcceso()`, no el atributo.
2. **`await nextTick()` antes del `focus()`** en el `finally`, para quien envía con el mouse: ahí el foco quedó en el botón, que se deshabilita solo al vaciarse `documento`.
3. **Handler global de `keydown`** (`onKeydownGlobal`): teclear un dígito desde cualquier parte devuelve el cursor al campo. Sin él, alcanza con que alguien haya clickeado el fondo para que lo que escriba el siguiente no vaya a ningún lado — y sin nadie en el mostrador eso se lee como "el sistema no anda". Solo dígitos sin modificadores (Tab, F5 y los atajos del navegador pasan de largo), y **no actúa con un modal abierto**: el de salida del kiosco tiene el campo de contraseña, que también se escribe con números. Agrega el dígito a mano con `preventDefault()` en vez de confiar en que el carácter caiga en el input recién enfocado —eso depende del navegador y si cae igual quedaría duplicado— y fija el cursor al final, porque si no los dígitos siguientes se intercalan al revés.
...
**Pendientes usa el mismo patrón que el resto: tabla en desktop, cards en móvil.** Fue una grilla de tarjetas grandes con los seis datos de afiliación a la vista; con 20 o 40 registros era una pared de scroll. Ahora la fila muestra solo lo que decide (cliente, contacto, fecha de registro) y el botón **Datos** despliega el resto —género, nacimiento, EPS, barrio, emergencia y, en menores, el bloque del acudiente— en `components/PendienteDetalle.vue`, compartido por la fila expandida y la card móvil. **No devolverlo a grilla de cards.**

**No hay "plan solicitado".** Existió un flujo donde el pendiente tocaba un plan en `/planes` y eso escribía `usuarios.plan_solicitado_id`, que el listado mostraba en una columna y el modal de activar usaba para precargar plan y monto. Se eliminó entero —columna de la tabla, `POST /planes/{id}/solicitar`, el campo en `/me` y en `/usuarios/pendientes`, y el precargado— porque **la asignación es empírica: el admin elige el plan cuando recibe el pago**, y la "solicitud" no era más que un dato que nadie miraba. El pendiente sigue viendo los planes y mandando el comprobante por WhatsApp, que es el camino real. Siguiendo el precedente de `ejercicios.categoria`, **la columna no se dropeó de la base**: el modelo dejó de mapearla y salió de los dos bloques de migración de `main.py`.
...
**`MarcasView.vue`** — listado de los ejercicios medibles del catálogo: **tabla en desktop, cards en móvil** (`sm:hidden` / `hidden sm:block`), mismo patrón que `EjerciciosView` y el listado de Clientes. Era un grid de cards grandes; con una docena de filas eran tres pantallas de scroll para lo que entra en una. **No devolverlo a grid.** Columnas: Ejercicio · Mejor marca · **Video** · Última. La columna Video reemplazó a *Registros* (el conteo de marcas): el video es accionable desde el listado y el conteo solo se miraba de paso — y además ya se ve dentro del ejercicio. El enlace lleva **`@click.stop`** porque la fila entera navega; en la card de móvil, donde el contenedor es un `RouterLink`, hace falta `.stop.prevent` + `window.open` a mano: un `<a>` anidado dentro de un `RouterLink` no navega solo. La fila entera es clicable (`@click` → `irA()`), pero el nombre va como `RouterLink` con `@click.stop` — así sigue siendo un enlace real (teclado, abrir en pestaña nueva) y el `.stop` evita la navegación duplicada.
...
**Consecuencia: el Resumen y el tab Clientes › Activos pueden discrepar.** `UsuariosView` sí mira los dos ejes (ver `tieneMembresia` más arriba), así que un socio con el bono agotado y fecha vigente cuenta como activo en `/dashboard` y como **inactivo** en `/usuarios`. Es deliberado y la diferencia es del lado correcto: la pantalla desde la que se le escribe al socio es la que tiene que decir la verdad sobre si puede entrar. Si algún día molesta, la salida es registrar el consumo de accesos, no volver a mirar un solo eje en `UsuariosView`.
...
**`REINICIAR-BRIDGE.cmd`** viaja en la misma carpeta y lo instala el mismo `.cmd` (son las dos herramientas de operación del bridge; un tercer instalador sería un paso más para olvidarse). Deja un acceso directo **"Reiniciar huellero"** en el escritorio **público** (`CommonDesktopDirectory`), para que aparezca sin importar qué usuario esté en recepción. Se auto-eleva —matar un proceso elevado desde una consola normal no se puede—, espera 3 s entre el `taskkill` y el arranque (el proceso tarda un instante en soltar 8001/8765 y el COM, y sin la pausa la instancia nueva los encuentra tomados y muere: el síntoma sería *"lo reinicié y ahora no anda"*), y al final verifica contra `/status` en vez de contra `tasklist` — porque "proceso vivo pero mudo" es justamente el modo de falla que cuesta ver.
