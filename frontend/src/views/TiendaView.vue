          ...
          <div v-for="p in productosActivos" :key="p.id"
            ...
            <div class="p-4">
              <div class="flex items-start gap-2 mb-1">
                ...
                <span v-if="p.categoria" class="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full whitespace-nowrap">{{ p.categoria }}</span>
              </div>
              <p class="text-xs text-gray-400 line-clamp-2 mb-3 min-h-[32px]">{{ p.descripcion || '' }}</p>

              <!-- El precio lo ven los dos: al cliente es lo primero que le hace falta
                   para decidir si lo pide. -->
              <div class="flex items-center justify-between gap-2">
                <span class="text-xl font-black text-red-600">${{ p.precio.toLocaleString('es-CO') }}</span>
                <span v-if="!canManage" class="text-xs font-semibold whitespace-nowrap"
              ...
              <!-- Pedido del cliente: no hay carrito ni checkout, el pedido se cierra
                   por WhatsApp con el admin. Condicionado al LINK y no al teléfono —
                   un número incompleto mandaría a una pantalla de error de WhatsApp
                   (ver "Teléfonos de WhatsApp" en CLAUDE.md). -->
              <template v-if="!canManage">
                <a v-if="p.stock > 0 && whatsappProducto(p)" :href="whatsappProducto(p)" target="_blank" rel="noopener"
                  class="mt-3 w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold py-2.5 rounded-lg transition-colors">
                ...
                <p v-else-if="p.stock > 0" class="mt-3 text-xs text-center text-gray-400">
                  Pregunta en recepción para pedirlo.
                </p>
                <p v-else class="mt-3 text-xs text-center text-gray-400">
                  Sin unidades por ahora.
                </p>
              </template>
            </div>
          </div>
...
// ── Pedido por WhatsApp (cliente) ─────────────────────────────
// No hay carrito ni checkout para el cliente: el pedido se cierra hablando con el
// admin, que es como ya funciona el pago de un plan. La venta la registra después
// el staff desde el POS, así que el stock no se toca desde acá.
const adminTelefono = ref('')

// El número se normaliza con `linkWa` (lib/whatsapp.js), la misma regla que el
// backend. Devuelve null si no sirve → la tarjeta esconde el botón en vez de
// mandar a una pantalla de error de WhatsApp.
function whatsappProducto(p) {
  const nombre  = localStorage.getItem('userName') || ''
  const mensaje = `Hola! Quiero pedir *${p.nombre}* - $${p.precio.toLocaleString('es-CO')} COP.` +
...
onMounted(async () => {
  await cargarProductos()
  // Solo el cliente necesita el teléfono: el staff no se pide cosas a sí mismo.
  if (!canManage.value) {
    try {
      adminTelefono.value = (await api.get('/contacto')).data.telefono || ''
    } catch { /* silencioso: sin teléfono, la tarjeta manda a recepción */ }
  }
})
</script>
